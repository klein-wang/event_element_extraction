...
def train_ccks2lic(s):
    t = {'text': '',
     ...
     'title': ''}
    t['id'] = 'NA'
    t['text'] = cut_sentences(s['originalText'])
    t['title'] = t['text'].split('\n')[0]
    t['event_list'][0]['trigger'] = 'NA'
    t['event_list'][0]['event_type'] = '疾病'
    for i in s['entities']:
        start = int(i['start_pos'])
        end = int(i['end_pos'])
        entity = s['originalText'][start:end]
        t['event_list'][0]['arguments'].append({'role': i['label_type'], 'argument': entity})
    return t
...
def test_ccks2lic(s):
    t = {'text':'','id':'','title':''}
    text = cut_sentences(s) # raw data with no keys (unlike in training data)
    t['text'] = text
    t['id'] = 'NA'
    t['title'] = text.split('\n')[0]
    return t
...
def gain_test(raw_path, save_path):
    ...
    with open(save_path,'w', encoding='utf-8') as f:
        for i,data in enumerate(ccks_test):
            new_data = test_ccks2lic(data) # remove eval() from initial ccks2lic.py
            json.dump(new_data,f,ensure_ascii=False)
            f.write('\n')


if __name__ == '__main__':
    train_raw_data_path = 'data/ccks/raw/task1_train.txt' # 和ccks2lic.py里的path不同
    test_raw_data_path = 'data/ccks/raw/task1_unlabeld_val.txt' # 和ccks2lic.py里的path不同
